...
class ModelCheckoutOrder extends Model {
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_product_query->rows as $order_product) {
					...
					foreach ($order_option_query->rows as $option) {
						$this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity - " . $order_product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND subtract = '1'");
					}
				}
			}
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach($product_query->rows as $product) {
					$this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . $product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");

					$option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$product['order_product_id'] . "'");

					foreach ($option_query->rows as $option) {
						$this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity + " . $product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND subtract = '1'");
					}
				}
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$mail->send();
				*/
				if(isset($this->session->data['order_confirmation_email']) && $this->session->data['order_confirmation_email'] != ''){
					$order_email = $this->session->data['order_confirmation_email'];
				} elseif($order_info['payment_email'] != ''){
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
				...
				*/
				...
				if($order_status_id == '3'){  //shipped..
					...
					$delivery_time = '';
					foreach ($order_product_query->rows as $order_product) {
						$delivery_date = $order_product['ship_date'];
						$delivery_time = $order_product['ship_time'];
						break;
					}
					...
					foreach ($order_total_query->rows as $total) {
						if($total['title'] == 'Total'){
							$delivery_cost = $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']);
						}
					}
	...
	public function saveDelivery($ship_date){
		
		$shipdate = date("Y-m-d",strtotime($ship_date));
		
		$this->db->query("UPDATE " . DB_PREFIX . "cart SET ship_date = '" . $shipdate . "' WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' ");
		
		return '1';
	}
	
	public function saveDeliveryTime($ship_time){
		
		$this->db->query("UPDATE " . DB_PREFIX . "cart SET ship_time = '" . $ship_time . "' WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' ");
	...
	public function getOrders() {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order WHERE order_status_id > 0 ");
		if($query->num_rows){
			return $query->rows;
		} else {
			return array();
	...
	public function getOrderOptions($order_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '".$order_id."' ");
		if($query->num_rows){
			return $query->rows;
		} else {
			return array();
	...
	public function getOrderProducts() {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product op LEFT JOIN " . DB_PREFIX . "order o ON op.order_id = o.order_id WHERE o.order_status_id > 0");
		if($query->num_rows){
			return $query->rows;				
		} else {
			return array();
