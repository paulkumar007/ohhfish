...
class ModelAccountCustomer extends Model {
	...
	public function checkMobile($mobile) {
		$check_mobile = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE (LOWER(email) = '" . $this->db->escape(utf8_strtolower($mobile)) . "' OR telephone = '" . $this->db->escape($mobile) . "') AND status = '1'");
	...
	public function createPassword($mobile,$password) {
		$update_pass = $this->db->query("UPDATE " . DB_PREFIX . "customer SET password = '".md5($password)."' WHERE (LOWER(email) = '" . $this->db->escape(utf8_strtolower($mobile)) . "' OR telephone = '" . $this->db->escape($mobile) . "') AND status = '1'");
