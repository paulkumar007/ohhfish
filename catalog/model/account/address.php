...
class ModelAccountAddress extends Model {
	public function addAddress($data) {
		...
		$get_customer = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$this->customer->getId() . "'");		if($get_customer->num_rows){			$customer = $get_customer->row;						if($customer['firstname'] == '' && $customer['lastname'] == ''){				$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "' WHERE customer_id = '" . (int)$this->customer->getId() . "'");			}		}
	...
	}		public function addOrderEmailId($order_email){				$get_customer = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$this->customer->getId() . "'");		if($get_customer->num_rows){			$customer = $get_customer->row;						if($customer['email'] == ''){				$this->db->query("UPDATE " . DB_PREFIX . "customer SET email = '" . $this->db->escape($order_email) . "' WHERE customer_id = '" . (int)$this->customer->getId() . "'");			}		}				return '1';	}
}