...
class ControllerAccountRegister extends Controller {
	...
	public function checkMobile() {
		...
		$customer_status = $this->model_account_customer->checkMobile($mobile);
		if($customer_status != '1') {
			...
			if ($send == "SUBMIT_SUCCESS "){
				$this->session->data['otp'] = $mobile.'|'.$random_no;
			} else {
				echo 'invalid';
				exit;
			}
			*/
			...
			$ch = curl_init("https://app.indiasms.com/sendsms/sendsms.php?username=".$username."&password=".$password."&type=TEXT&sender=".$sender_id."&mobile=".$mobile."&message=".urlencode($message));
			 ...
			 if(isset($output_arr[0]) && trim($output_arr[0]) == 'SUBMIT_SUCCESS'){
				 $this->session->data['otp'] = $mobile.'|'.$random_no;
			 } else {
				 echo 'invalid';
				 exit;
	...
	public function loginMobile(){
		...
		if($customer_status == '0') {
			...
			if(filter_var($mobile, FILTER_VALIDATE_EMAIL)) {
				...
				$this->model_catalog_email_manager->sendEmail($data,'email-login-otp',$mobile);
				echo '0';
	...
	public function verifyOTP() {
		...
		if(isset($this->session->data['otp'])){
			$old_otp = $mobile.'|'.$otp;
			if($this->session->data['otp'] == $old_otp){
	...
	public function verifyOTPLogin() {
		...
		if(isset($this->session->data['otp'])){
			...
			if($this->session->data['otp'] == $old_otp){
				unset($this->session->data['otp']);
				$this->customer->registerUser($mobile);
				echo '1';
			} else {
				echo '0';
			}
	...
	public function verifyPasswordLogin() {
		...
		if (!$this->customer->login($mobile,$pass,false)) {
			echo '0';
		} else {
			echo '1';
		}
	...
	public function forgotMobile(){
		...
		if($customer_status == '1') {
			...
			if(filter_var($mobile, FILTER_VALIDATE_EMAIL)) {
				...
				$this->model_catalog_email_manager->sendEmail($data,'email-forgot-otp',$mobile);
				$this->session->data['otp'] = $mobile.'|'.$random_no;
				echo '1';
			} elseif(preg_match('/^[0-9]{10}+$/', $mobile)) {
				/* SEND SMS */
	...
	public function verifyForgotOTPLogin() {
	...
	public function createNewPassword(){
		...
		$this->model_account_customer->createPassword($mobile,$password);
		if (!$this->customer->login($mobile,'',true)) {
			echo '2';
		} else {
			echo '1';
		}
