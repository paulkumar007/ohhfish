...
class ControllerAccountReturn extends Controller {
	...
	public function add() {
		...
		$data['accept_return'] = 'no';
		if (isset($this->request->get['order_id'])) {
			$order_info = $this->model_account_order->getOrder($this->request->get['order_id']);
			
			if($order_info['date_added'] != '' && $order_info['date_added'] != '0000-00-00 00:00:00'){
				$order_time = strtotime($order_info['date_added']);
				$current_time = time();
				$time_diff = $current_time - $order_time;
				if($time_diff <= 1800){
					$data['accept_return'] = 'yes';
				}
			}
		}
