...
class ControllerCheckoutPaymentAddress extends Controller {
	...
	public function addEmailAddress() {
		...
		if (isset($this->request->post['order_email_id'])) {
			$order_email_id = $this->request->post['order_email_id'];
		} else {
			$order_email_id = '';
		}
		
		$this->session->data['order_confirmation_email'] = $order_email_id;
		
		$this->load->model('account/address');
		$order_email_id = $this->model_account_address->addOrderEmailId($order_email_id);
